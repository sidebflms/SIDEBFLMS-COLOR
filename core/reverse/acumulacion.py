...
def acumular_correspondencias(
    ...
    * `cobertura` es el `CoverageMap`: `counts` = cuantos pixeles usan cada nodo
    ...
    celdas = n**3
    acumulado = np.zeros((celdas, 4), dtype=np.float64)

    if src.shape[0] == 0:
        return (
            acumulado.reshape(n, n, n, 4),
            CoverageMap(
    ...
    suma_r = np.zeros((celdas, 3), dtype=np.float64)
    suma_r2 = np.zeros((celdas, 3), dtype=np.float64)
    for k in range(8):
        wk = w[k]
        vive = wk >= PESO_DE_MUESTRA
        counts += np.bincount(idx[k][vive], minlength=celdas)
        acumulado[:, 3] += np.bincount(idx[k], weights=wk, minlength=celdas)
        for c in range(3):
            acumulado[:, c] += np.bincount(idx[k], weights=wk * dst[:, c], minlength=celdas)
            suma_r[:, c] += np.bincount(idx[k], weights=wk * residuo[:, c], minlength=celdas)
            suma_r2[:, c] += np.bincount(idx[k], weights=wk * residuo[:, c] ** 2, minlength=celdas)

    peso = np.maximum(acumulado[:, 3], 1e-12)[:, None]
    varianza = np.maximum(suma_r2 / peso - (suma_r / peso) ** 2, 0.0).mean(axis=1)
    varianza[counts < 2] = 0.0  # con una muestra no hay varianza que medir

    cobertura = CoverageMap(
        counts=counts.reshape(n, n, n).astype(np.int32),
        variance=varianza.reshape(n, n, n).astype(np.float32),
        min_samples=int(min_muestras),
    )
    return acumulado.reshape(n, n, n, 4), cobertura
